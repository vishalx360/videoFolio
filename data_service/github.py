...
def get_github_data(username):
    ...
    cont_data=requests.get(url).text
    soup = BeautifulSoup(cont_data, 'lxml')
    ...
    if(len(popular_repo)==0):    
        ...
        data_dict={'name':data['name'],'bio':data['bio'],'avatar':data['avatar_url'],'repo_count':len(repo),'company':data['company'],'popular_repo':popular_repo,'contribution':temp[0],'language':list_lang,'total_contributed_repo':len(contributed_repo),'popular_contribution':list_fork,'twitter':data['twitter_username']}
